...
<form action="/agregar" method="post" enctype="multipart/form-data" >
    ...
    <div class="form-group">
        ...
        <small id="comp_mail"  class="text-danger"></small>
...
<script>
    $('#email').blur(function(){
        ...
        if(!filter.test(email))
        {
            ...
            $('#comp_email').html('<small class="text-danger">Correo Invalido</small>');
        ...
        else
        {
            $.ajax({
               ...
               success:function(result)
               {
                   if(result=='unique')
                   {
                       $('#comp_email').html('<label class="text-success">Email Disponible</label>');
                       $('#email').removeClass('has-error');
                       $('#register').attr('disabled',false);
                   }
                   else
                   {
                    $('#comp_email').html('<small class="text-danger">Email en uso</small>');
